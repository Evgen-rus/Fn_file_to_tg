...
- Статус клиента -> из БД leads.status
- Телефон (мобильный) осн.контактного лица -> из БД leads.phone
...
Остальные колонки создаются пустыми, но присутствуют в файле.
"""
...
DEFAULT_TARIFF_REMAINING = 10000
...
HEADERS: List[str] = [
    ...
    'Должность основного контактного лица',
    'Телефон (стационарный) осн.контактного лица',
    'Телефон (мобильный) осн.контактного лица',
    'E-mail',
    'Регион клиента',
    'Вид деятельности',
    'Сумма предодобренного лимита',
    'Условия предоставления лимита',
    'Лизинговая компания',
    'Специалист КЦ',
    'UTM_CAMPAIGN',
    'Направление',
...
def build_workbook(rows: List[Tuple[str, str, str, str]]) -> Workbook:
    ...
    # Индексы колонок, которые заполняем
    idx_status = HEADERS.index('Статус клиента') + 1
    idx_mobile = HEADERS.index('Телефон (мобильный) осн.контактного лица') + 1
    ...
    for phone, utm_campaign, direction, status in rows:
        row = [''] * len(HEADERS)
        row[idx_status - 1] = status or 'Пасивный'
